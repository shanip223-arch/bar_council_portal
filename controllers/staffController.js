...
async function getStaffDashboard(req, res) {
  try {
    ...
    const recent_obj = await pool.query(`
      SELECT o.id, o.type, o.deadline, o.resolved, a.application_no, a.name
      FROM objections o JOIN applications a ON a.id=o.application_id
      ORDER BY o.created_at DESC LIMIT 5
    `);
    const recent_upl = await pool.query(`
      SELECT u.id, u.doc_type, u.review_status, u.uploaded_at, a.application_no, a.name
      FROM uploads u JOIN applications a ON a.id=u.application_id
...
async function getStaffObjections(req, res) {
  try {
    ...
    const countRes = await pool.query(
      `SELECT COUNT(*) FROM objections o JOIN applications a ON a.id=o.application_id ${wc}`, params
    );
    const rows = await pool.query(
      `SELECT o.id, o.type, o.remark, o.deadline, o.resolved, o.staff_remark, o.created_at,
              a.id AS app_id, a.application_no, a.name, a.district,
...
async function getStaffUploads(req, res) {
  try {
    ...
    const wc = where.length ? 'WHERE ' + where.join(' AND ') : '';
    const countRes = await pool.query(
      `SELECT COUNT(*) FROM uploads u JOIN applications a ON a.id=u.application_id ${wc}`, params
    );
    const rows = await pool.query(
      `SELECT u.id, u.doc_type, u.original_name, u.file_name, u.file_path, u.section_code,
              u.review_status, u.staff_remark, u.verified, u.uploaded_at,
              a.id AS app_id, a.application_no, a.name, a.district,
