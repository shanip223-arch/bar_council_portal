...
async function uploadObjectionDoc(req, res) {
  ...
  try {
    ...
    const uploaded = await pool.query('SELECT DISTINCT objection_id FROM uploads WHERE application_id=? AND section_code=?', [app.id, 'obj']);
    ...
    const exists = await pool.query('SELECT id,file_path FROM uploads WHERE application_id=? AND objection_id=? AND section_code=?', [app.id, objection_id, 'obj']);
    if (exists.rows.length) {
      if (fs.existsSync(exists.rows[0].file_path)) fs.unlinkSync(exists.rows[0].file_path);
      await pool.query('UPDATE uploads SET original_name=?,file_name=?,file_path=?,uploaded_at=NOW() WHERE id=?', [req.file.originalname, fileName, destPath, exists.rows[0].id]);
    } else {
      await pool.query(
    ...
    if (unresolved.length === uploaded.rows.length + 1 && app.final_chance_used === 0) {
      await pool.query('UPDATE applications SET final_chance_used=1,status=? WHERE id=?', ['under_review', app.id]);
    }
